...
if len(sys.argv) ==3:
	...
	if search("\.pcap",LOG_FILE_NAME,I):
		split_option=raw_input('Do you want to split the pcap file ? Y/N')
		if match('Y',split_option,I):
			split_size=raw_input('Enter Split size in MB : ')
			print str(datetime.now())
			system("python Fapi_spilt.py "+LOG_FILE_NAME+" "+split_size)
			List_pcap=listdir(LOG_FILE_NAME.split(".")[0]+"/")
		else:
			List_pcap=[LOG_FILE_NAME]
...
CRC_Message={}
if File_pcap:
	LOG_FILE_FOLDER=LOG_FILE_NAME.split(".")[0]
for FL in List_pcap:
	row=1;SIB_Comment=0;MIB_Comment=0
	worksheet_count=0;Frame=0;
	if File_pcap:
		LOG_FILE_NAME=LOG_FILE_FOLDER+"\\"+FL
	else:
		LOG_FILE_NAME=FL
	if search("\.pcap",LOG_FILE_NAME,I):
		LOG_FILE_NAME=pcap2textConvert(LOG_FILE_NAME)
		...
		with open(LOG_FILE_NAME,"r") as LOG_FH:
			...
			for line in LOG_FH:
				...
				if search("Frame Number:",line,I):
					...
					if Msg_id=="0x80" :
						row,M=DL_CONFIG_REQ(LOG_FH,Frame,worksheet,row,format1,SYS_BW)
						if 'PDUs' in M and len(M['PDUs']) >0 :
							if len(DL_Q) >=4:
								DL_Q.pop()
							DL_Q.insert(0,M.copy())
							#print DL_Q
							del M
					elif Msg_id=="0x81":
						...
						row=HARQ_IND(LOG_FH,Frame,worksheet,row,format1,UL_Q,DL_Q)
					elif Msg_id=="0x86":
